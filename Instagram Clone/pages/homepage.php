...
// Get the user's email and rcsid from the session
$rcsid = $_SESSION['rcsid'];

// Query to find users not followed by the current user
$query = "SELECT u.first_name, u.last_name, u.rcsid 
          FROM users u 
          WHERE u.rcsid NOT IN (
              SELECT f.following_id 
              FROM followers f 
              WHERE f.follower_id = '{$rcsid}'
          ) AND u.rcsid != '{$rcsid}'";
...
// Fetch user details from account_details table
...
<div class="message-panel">
    ...
    <div class="message-content">
      ...
      <form id="message-form" action="./messages/send_message.php" method="POST">
...
<script>
  ...
  function updateMiddleSection(navType) {
    ...
    switch(navType) {
      ...
      case 'Notifications':
        middleSection.innerHTML = '<p>This is the Notifications content.</p>';
        break;
      case 'Following':
